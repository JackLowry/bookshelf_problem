...
asset_options = gymapi.AssetOptions()
asset_options.fix_base_link = True
shelf_asset = gym.load_asset(sim, "/home/aurmr/isaacgym/aurmr_grasping_research/assets/", "shelf/shelf.urdf", asset_options)

shelf_side3_idx = gym.find_asset_rigid_body_index(shelf_asset, "shelf_side3")
shelf_sensor_pose = gymapi.Transform(gymapi.Vec3(0.2, 0.0, 0.0))
shelf_sensor_idx = gym.create_asset_force_sensor(shelf_asset, shelf_side3_idx, shelf_sensor_pose)
...
table_pose.p = gymapi.Vec3(0.05, 0.0, 0.5 * table_dims.z)

shelf_pose = gymapi.Transform()
shelf_pose.p = gymapi.Vec3(-.7, 0, .8)
shelf_pose.r = gymapi.Quat(0, 1/math.sqrt(2), 0.0, 1/math.sqrt(2))
...
for i in range(num_envs):
    ...
    for i in range(num_sensors):
        ...
        print(sensor)
    # add box
    box_pose.p.x = shelf_pose.p.x
    box_pose.p.y = shelf_pose.p.y
    box_pose.p.z = .5 + 0.5 * box_size
...
approach_angle = torch.rand(num_envs, 1).to(device)*2*math.pi
box_approach_x = torch.cos(approach_angle)
box_approach_y = torch.sin(approach_angle)
box_approach = torch.hstack((box_approach_x, box_approach_y))*.1
cartesian_steps = 100
...
while not gym.query_viewer_has_closed(viewer):
    ...
    start_pos[:, :2] = box_pos[:, :2] + box_approach

    end_pos = box_pos.clone()
    end_pos[:, :2] = box_pos[:, :2] - box_approach

    if cartesian_traj is None:
        cartesian_traj = tensor_linspace(start_pos, end_pos, steps=cartesian_steps)
    print(cartesian_traj)

    to_goal = cartesian_traj[:, :, cartesian_idx].squeeze(-1) - hand_pos
    goal_dist = torch.norm(to_goal, dim=-1)
    hand_speed = torch.norm(hand_vel, dim=-1)


    new_go_push = (~go_push & (goal_dist < 0.025) & (hand_speed < 0.02)).squeeze(-1)
    push_pos = box_pos.clone()
    go_push = go_push | new_go_push
    
    new_push_pos = box_pos.clone()
    new_push_pos[:, :2] = new_push_pos[:, :2] - box_approach

    push_pos = torch.where(go_push & new_go_push, new_push_pos, push_pos)

    new_cartesian_traj = tensor_linspace(start_pos, end_pos, steps=cartesian_steps)
    cartesian_traj = torch.where(cartesian_idx==0, new_cartesian_traj, cartesian_traj)
    print(go_push & new_go_push & (cartesian_idx==0))
    cartesian_idx = torch.where(go_push & (goal_dist < 0.022) & (cartesian_idx<cartesian_steps-1),  cartesian_idx+1, cartesian_idx)
    print(cartesian_idx)


    # compute goal position and orientation
    goal_pos = cartesian_traj[:, :, cartesian_idx].squeeze(-1)
    goal_rot = torch.where(go_push, init_rot, quat_mul(down_q, quat_conjugate(yaw_q)))
